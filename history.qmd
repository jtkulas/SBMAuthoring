...
>I'm confident that we will be able to expect each member of the next generation of programmers to be familiar with a document language as well as a programming language [@knuth84, p. 2]

@knuth1984texbook developed [$\TeX$](https://visualmatheditor.equatheque.net/doc/texbook.pdf) in 1978 along with a companion language to design and generate digital fonts, [`METAFONT`](https://veryinteractive.net/pdfs/knuth-theconceptofameta-font.pdf)^["[Computer Modern](https://www.fontsquirrel.com/fonts/computer-modern)" is a font created by `METAFONT` -- parameter values define the nature of transitioning from one letter style specification to another (e.g., same control parameters, different values)] [{{< iconify twemoji:mushroom >}} @knuth1982concept {{< iconify twemoji:mushroom >}}] with a primary purpose of improving the formatting of mathematical expressions in his books. $\TeX$ defined spacing and layout whereas `METAFONT` provided fonts to use within the $\TeX$--specified pagespace. @lamportlatex developed $\LaTeX$, with a user's manual appearing in 1986. Both $\TeX$ and $\LaTeX$ are considered typesetting systems -- $\LaTeX$ provides macros that make $\TeX$ a bit more user-friendly for full document preparation and is the more common system encountered today. Both Knuth and Lamport were affiliated with the Stanford Research Institute {{< iconify mdi:head-idea-outline >}}.

`WEB` was introduced in 1981 and presented as a *literate programming system* (facilitating the writing of both computer code and documentation).^[Although the genesis of *literate programming* was within specific computer--science applications, the integration of code and narration is the critical characteristic that we are extracting as elements that can be leveraged by broader audiences (such as social--, behavioral--, and managerial-- or, more inclusively, data--scientists ).] `WEB` marries two primary components: a formatting language (originally $\TeX$) and a programming language (originally PASCAL; see [@knuth84](https://www.cs.tufts.edu/~nr/cs257/archive/literate-programming/01-knuth-lp.pdf))^[@knuth84 states, regarding other possible languages, that "...the same principles would apply equally well if other languages were substitued... ...the main point is that `WEB` is inherently bilingual, and that such a combination of languages proves to be much more powerful than either single language by itself." p. 2]. `WEB` both `TANGLE`s and `WEAVE`s -- *tangling* being the computational processing while *weaving* being the prose--y narration. `NoWeb` is an alternative early literate programming tool for statisticians [@ramsey1994literate], and is perhaps the origin of the term chunk (as in documentation chunk or code chunk). `Noweb` was inspired by `WEB`, but had no language specificity . `NoWeb` had similar tools: `notangle` and `noweave`. @rossini2001 coined the term "[Literate Statistical Practice](https://www.r-project.org/conferences/DSC-2001/Proceedings/Rossini.pdf)" -- his background being statistics, whereas Knuth is a computer scientist. 
...
Another representation of the history of literate programming can be found in a 2023 presentation [@allaire2023, see @vid-allaire2] 

::: {#vid-allaire2}

{{< video https://youtu.be/nllKcuX7rEc start="135" >}}
